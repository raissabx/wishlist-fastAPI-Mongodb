...
class TestProductAPI:
    ...
    def create_product(self) -> ProductAPIModel:
        product = {
            ...
            'image': 'http://challenge-api.luizalabs.com/images/c0aec577-f102-4ac8-470c-3d84fa8fd094.jpg',  # noqa
            'price': 51.66
        }
    ...
    async def test_add_favorite_product(
        ...
        response = test_app.put(
            f'/products_api/{customer_email}/{product_id}/',
            headers={'Authorization': f'Bearer {mock_create_jwt_token}'}
        )
    ...
    async def test_remove_all_favorite(
        ...
        assert response_data.get('detail') == 'Favorites list cleared successfully'  # noqa
    ...
    async def test_remove_favorite(
        ...
        mock_verify_token.return_value = mock_create_jwt_token
        response = test_app.delete(
            f'/products_api/{customer_email}/remove_favorites/{product_id}',
            headers={'Authorization': f'Bearer {mock_create_jwt_token}'}
        )
        ...
        response_data = json.loads(response.text)
        assert response_data.get('detail') == 'Product removed from favorites successfully'  # noqa
